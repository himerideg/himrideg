...
async function duplicateWalletTransactionCount() {
  ...
  const rows = await DriverPayoutMethod.aggregate([
    {
      $project: {
        ...
        key: {
          $cond: [
            { $eq: ["$type", "upi"] },
            { $ifNull: ["$upiFingerprint", ""] },
            { $ifNull: ["$bankFingerprint", ""] }
          ]
        }
      }
    },
...
async function unsecuredPayoutMethodCount() {
  return DriverPayoutMethod.countDocuments({
    $or: [
      { secretVersion: { $lt: 1 } },
      { secretVersion: { $exists: false } }
    ]
  });
}

async function missingPayoutSecretCount() {
  const rows = await DriverPayoutMethod.aggregate([
    {
      $lookup: {
        from: PayoutMethodSecret.collection.name,
        localField: "_id",
        foreignField: "payoutMethod",
        as: "__secret"
      }
    },
    {
...
async function legacyPlaintextDriverPayoutCount() {
  return User.countDocuments({
    role: "driver",
    $or: [
      { "driverProfile.bankDetails.accountNumber": { $nin: [null, ""] } },
      { "driverProfile.bankDetails.upiId": { $nin: [null, ""] } }
    ]
  });
}

async function runDatabaseIntegrityAuditV2() {
  ...
  const [
    ...
    PayoutMethodSecret.countDocuments({}),
    RideAutoPayout.countDocuments({}),
    ...
    duplicateWalletTransactionCount(),
    ...
    unsecuredPayoutMethodCount(),
    missingPayoutSecretCount(),
    legacyPlaintextDriverPayoutCount()
  ]);
  ...
  const summary = {
    ...
    totals: {
      ...
      payoutSecrets: totalPayoutSecrets,
      rideAutoPayouts: totalRideAutoPayouts
    },
    issueTotal,
    issues
  };
  ...
  console.log(
    `🔎 DB_INTEGRITY_AUDIT_V2 ${JSON.stringify(summary)}`
  );
