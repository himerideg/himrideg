...
function safePush(userId, options) {
...
async function markDriverArriving({
  ...
  safePush(populatedBooking.customer, {
    title: "Driver is Coming 🚕",
    body: "Fare locked hai. Driver aapke pickup ki taraf aa raha hai.",
    data: {
      type: "driver_arriving",
      soundEvent: "driver_arriving",
      role: "customer",
      bookingId: String(populatedBooking._id),
...
async function markDriverArrived({
  ...
  /*
  | Arrival only moves the ride into driver_arrived state and clears any stale
  | legacy OTP. A fresh OTP is created only when the assigned driver explicitly
  | taps Generate OTP after reaching the customer.
  */
  ...
  /* Arrival notification is status-only. No plain OTP exists until the
     assigned driver explicitly generates one after arrival. */
  safeEmit(
    emitDriverArrived,
    {
      booking
...
async function startRide({
  ...
  safePush(populated.customer, {
    title: "Ride Started 🚕",
    body: "Aapki HimRideG ride start ho gayi. Live route destination tak active hai.",
    data: {
      type: "ride_started",
      soundEvent: "ride_started",
      role: "customer",
      bookingId: String(populated._id),
      status: populated.status
    }
...
async function completeRide({
  ...
  */
  ...
  const completedFare = Number(
    booking.finalFare || booking?.fare?.finalFare || booking.driverOfferedFare || 0
  );

  safePush(booking.customer, {
    title: "Ride Completed ✅",
    body: booking.paymentStatus === "paid"
      ? `Ride complete. ₹${completedFare} payment successful hai.`
      : `Ride complete. Final fare ₹${completedFare}. Ab payment method choose karein.`,
    data: {
      type: "ride_completed",
      soundEvent: booking.paymentStatus === "paid" ? "ride_completed" : "payment_required",
      role: "customer",
      bookingId: String(booking._id),
      status: booking.status,
      fare: completedFare,
  ...
  safePush(booking.driver, {
    title: "Ride Completed",
    body: booking.paymentStatus === "paid"
      ? `₹${completedFare} payment received.`
      : `Ride complete. ₹${completedFare} payment confirmation pending hai.`,
    data: {
      type: "ride_completed",
      soundEvent: "ride_completed",
      role: "driver",
      bookingId: String(booking._id),
      status: booking.status,
      fare: completedFare,
