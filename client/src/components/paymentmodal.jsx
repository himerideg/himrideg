...
function PaymentModal({ booking, onSuccess, onClose }) {
  ...
  */
  ...
  const reportPaymentFailure = useCallback(async (reason, details = {}) => {
    try {
      ...
      // Failure audit network error customer payment UI ko block na kare.
    }
  }, [bookingId]);
  ...
  const handleOnlinePayment = useCallback(async () => {
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            const message = err?.response?.data?.message || err.message || "Payment verify karne mein error";
            await reportPaymentFailure(message, { stage: "verify" });
            setError(message);
            setStep("error");
          }
        }
      ...
      razorpay.on("payment.failed", async (response) => {
        const message = response.error?.description || "Payment fail ho gayi. Dobara try karo.";
        await reportPaymentFailure(message, {
          stage: "checkout",
      ...
      const message = err.response?.data?.message || err.message || "Payment start nahi ho saka";
      await reportPaymentFailure(message, { stage: "create_order" });
      setError(message);
      setStep("error");
    } finally {
      setLoading(false);
    }
  }, [bookingId, booking, onSuccess, reportPaymentFailure]);
  ...
  */
  const handleCashPayment = useCallback(async () => {
    ...
    try {
      // Backend par cash selection save karo. Driver receive karne ke baad confirm karega.
      const { data } = await api.post("/payments/cash-select", { bookingId });
      if (!data?.success) throw new Error(data?.message || "Cash payment select nahi ho saka");
      ...
      setError(err?.response?.data?.message || err.message || "Cash payment select nahi ho saka");
      setStep("error");
    } finally {
      setLoading(false);
    }
  }, [bookingId, finalFare, onSuccess]);
