...
/*
|=============================================================================
| V75 WEBSITE + MOBILE BROWSER PARITY
|=============================================================================
| FULL CODE RULE:
| - V72/V74 existing app-parity, ride, fare, OTP, map, payment and account code
|   stays preserved.
| - This script runs AFTER V72 + V74 and adds only the requested V75 behavior.
| - It is idempotent and refuses unsafe partial patching if an anchor drifts.
|=============================================================================
*/

/* --------------------------------------------------------------------------
 * Driver dashboard: clean History, clean Earnings/Platform Fee, ₹100 gate,
 * contact unlock only after Go to Pickup, same behavior desktop + mobile web.
 * -------------------------------------------------------------------------- */
{
...
/* --------------------------------------------------------------------------
 * Customer dashboard: phone/message stay locked until driver taps Go to Pickup.
 * -------------------------------------------------------------------------- */
{
...
/* --------------------------------------------------------------------------
 * Customer payment modal: temporary direct-to-driver UPI option.
 * -------------------------------------------------------------------------- */
{
...
/* --------------------------------------------------------------------------
 * Driver payment popup: direct UPI claim must be confirmed by the driver,
 * never by the customer's claim alone.
 * -------------------------------------------------------------------------- */
{
  ...
  if (!source.includes(marker)) {
    ...
    source = source.split('{busy === "cash" ? "Confirming…" : `Cash Received ${money(due)}`}').join('{busy === "cash" ? "Confirming…" : directDriverUpi ? `UPI Payment Received ${money(due)}` : `Cash Received ${money(due)}`}');
    source = source.replace(
...
/* --------------------------------------------------------------------------
 * Map: route bounds must always include Destination pin, even while pickup-only
 * route data is the first route to load.
 * -------------------------------------------------------------------------- */
{
